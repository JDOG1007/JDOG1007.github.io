...
class ButtonClass{
 ...
 function buttonCommand() {
   ...
   if(buttonName == 'DELETE'){
     ...
     tw.length = tw.length - 1
     o -= 1
     
    fill('white')
       rect(60+(tw.length)*70,170+(ln-1)*50,35, 35)
     return
     }
   ...
   textSize(25)
   text(buttonName, (o*70)-2, (ln*50)+ 147)
   x2 += 1
...
function checkWord(w){
    TS = word  
     ...
     if(words.indexOf(w.toLowerCase()) == -1 && words1.indexOf(w.toLowerCase()) == -1){
       ...
       for(let i = 0; i < 5; i++){
         if(TS[i] == tw[i]){
         fill(0, 255, 0);
           b[ord(TS[i])].buttonName.style("color:lime");
           styleBoolean[ord(tw[i])] = 1
           TS = TS.replaceAt(i, '+');
          textSize(25) 
         text(tw[i],68+(i*70),(ln*50)+ 147)
         }
       ...
       if(TS[0] == '+' && TS[1] == '+' && TS[2] == '+' && TS[3] == '+' && TS[4] == '+'){
         ...
         textSize(200)
         text('Win', 40, 400)
         restart = new ButtonClass("Restart?", "Restart?")
         restart.buttonName.position(400, 225)
       }
   ...
   for(let i = 0; i < 5; i++){
     if(TS.indexOf(tw[i]) != -1 && TS[i] != '+'){
    fill('orange');
       if( styleBoolean[ord(tw[i])] == 0){ 
       b[ord(tw[i])].buttonName.style("color:orange");
       }
     TS = TS.replace(tw[i], '+')
     textSize(25)
       text(tw[i],68+(i*70),(ln*50)+ 147)
         ...
         for (i = 0; i <= 4; i++) {
            if(word.indexOf(tw[i]) == -1){
              b[ord(tw[i])].buttonName.style("color:red");
            }
          }
...
function squares(n){
  ...
  fill('white')
  for(let i = 0;i<5;i++){
    rect(60+i*70,170+(n-1)*50,35, 35)
  }
}

function OutputButtons() {
  //let ButtonCount = 0;
  let k = [81, 87, 69, 82, 84, 89, 85, 73, 79, 80];
  for (let m = 0; m < 10; m++) {
    b[k[m] - 65] = new ButtonClass(char(k[m]), char(k[m]));
    b[k[m] - 65].buttonName.position(88 + 28 * m, 90);
  }
  let j = [65, 83, 68, 70, 71, 72, 74, 75, 76];
  for (let m = 0; m < j.length; m++) {
    b[j[m] - 65] = new ButtonClass(char(j[m]), char(j[m]));
    b[j[m] - 65].buttonName.position(104 + 28 * m, 115);
  }
  let t = [90, 88, 67, 86, 66, 78, 77];
  for (let m = 0; m < t.length; m++) {
    b[t[m] - 65] = new ButtonClass(char(t[m]), char(t[m]));
    b[t[m] - 65].buttonName.position(128 + 28 * m, 140);
  }
  enter = new ButtonClass("ENTER", 'ENTER')
enter.buttonName.position(400, 175)
delkey = new ButtonClass('DELETE', 'DELETE')
 delkey.buttonName.position(400, 225)

  //  button = new ButtonClass("button");
  //button.buttonName.position(400, 400);
} //outputbuttons
let word = "yeo"
function setup() {
...
createCanvas(500,500)
 background('GREY')
  ...
  OutputButtons()
 x=round(random(words1.length))
 
 fill(0, 0, 255)
  // text(words[x],100,90)//see how we index our 
 word = words1[x]
 ...
 //example of code using indexOf command
 
  for(let j = 1; j < 7; j++){
  squares(j)
  }
}//setup
function buttonCommand(ch) {
